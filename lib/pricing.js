...
class Pricing {
    ...
    static prices(currentHands) {
        ...
        if (this.isFlush() && this.isIncluded(numbers, [9, 10, 11, 12, 0]) === 5) { // 同花大順
            bettingOdds = 500;
        }
        else if (this.hasNumber() === 2 && suits.includes(4)) { // 五梅
            bettingOdds = 250;
        }
        else if (this.isFlush() && this.isStraight()) { // 同花順
            bettingOdds = 100;
        }
        ...
        else if (this.isFlush()) { // 同花
            bettingOdds = 7;
        }
        else if (this.isStraight()) { // 順子
            bettingOdds = 5;
        }
        else if ((this.hasNumber() === 3 && this.maxRepeat() === 3)
            || (this.hasNumber() === 4 && this.maxRepeat() === 2 && jokerNum === 1)
            || (this.hasNumber() === 4 && this.maxRepeat() === 1 && jokerNum === 2)) { // 三條, 113 or 1112+joker or 11111+2joker
            bettingOdds = 3;
        }
